...
int main () {
    ...
    int cents_deposited = 0, total_quarters = 0, total_dimes = 0, total_nickles = 0, total_pennies = 0;
    double total_after_fee = 0.00, total_before_fee = 0.00;
    ...
    /*Calculate the amount in dollars/cents after fee is applied
      Note: Questionable way to round, since it doesn't account for actual rounding rules, just truncates, which 
      is why TD Bank is paying out money from a lawsuit for its coin counting machines
    */
    
    total_before_fee = cents_deposited / 100.0;
    total_after_fee = static_cast<int>((cents_deposited - (cents_deposited * usage_fee))) / 100.0;
    ...
    //cout << endl << test << endl;
    cout << endl << "The total amount deposited: $" << total_before_fee << endl;
    cout << endl << "Total cash after fee: $" << total_after_fee << endl << endl;
...
/*
      //These equations work the same to figure out exact change, but didn't utilize modulo:
      ...
      //These equations work the same to figure out dollars and cents for total amount deposited, but were more convoluted
